...
const missionData = [
  ...
  },
  {
    ...
    items: ["Scalable Solutions", "Customer Listening", "Ship only what delivers value"],
  },
  {
    title: "What Is Most Important Now?",
    items: ["Build SnapFix MVP", "Launch website", "Plan go-to-market"],
  },
];
              ...
              <div key={index} className="flex flex-col lg:flex-row gap-6">
                {[left, right].map((card, i) => (
                  <div
                    ...
                    className={`${
                      i === 0 ? "lg:w-[60%]" : "lg:w-[40%]"
                    } bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/20 shadow-md`}
                  >
                    <h2 className="text-xl font-bold mb-2">{card.title}</h2>
                    {card.content && <p className="text-gray-300">{card.content}</p>}
                    {card.items && (
